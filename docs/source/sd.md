...
* Реалізує методи для швидкого сканування вмісту каталогу на карті пам'яті, в тому числі з фільтрацією по розширенню файлу.
* Автоматично використовує PSRAM, якщо доступна.
* Усі операції є потокобезпечними.
* Копіювання та видалення виконуються в паралельній задачі, завдяки чому GUI не блокується.
...
Замість стандартного FileStream, що доступний в Arduino, за потреби використовуйте клас з файлу `pixeler/src/manager/file/FileStream.h`.
...
В ньому необхідно вказати номер пристрою шини SPI або слот SD_MMC, що буде використовуватись для обміну даними з картою пам'яті, номери пінів, до яких підключена шина, її частоту тощо.

::: {note}
На різних моделях ESP номери пристроїв SPI можуть відрізнятися.
...
Після завершення роботи з файлом, його потрібно обов'язково самостійно закрити.
 ...
 * @param seek_pos Позиція від початку бінарного файлу у байтах, 
 ...
 * @param to Шлях до бінарного файлу, вказаний без точки монтування, 
...
Приклад використання з асинхронним оновленням смужки прогресу:

```cpp

// Метод-обробник зворотнього виклику прогресу копіювання
...
// Метод-обробник зворотнього виклику завершення задачі
void FilesContext::taskDoneHandler(bool result, void* arg)
{
  FilesContext* self = static_cast<FilesContext*>(arg);
  self->post([self, result]()
             { self->handleTaskDone(result); }, 500);
}

// Підписалися на зміну прогресу копіювання
_fs.onCopyProgress(copyProgressHandler, this);
// Підписалися на завершення задачі
_fs.onTaskDone(taskDoneHandler, this);

// Намагаємося запустити задачу копіювання
if (!_fs.startCopyingFile(old_file_path.c_str(), new_file_path.c_str()))
  showToast(STR_FAIL, TOAST_LENGTH_SHORT); // У випадку невдачі виводимо Toast
else
  showCopyingTmpl(); // У разу запуску задачі, завантажуємо шаблон копіювання

// Тепер прогресбар буде оновлюватися автоматично в процесі копіювання файлу.
// При цьому кнопки не будуть заблоковані, 
// а користувач матиме можливість відмінити копіювання до його завершення.
```
<br>

Задачу копіювання файлу може бути скасовано.
...
Перейменування та переміщення це одна і та ж операція, яку може бути застосовано однаково як до файлу так і до каталогу. 
