...
namespace pixeler
{
  bool FServer::_parseRequest(NetworkClient& client)
  {
  ...
  bool FServer::_collectHeader(const char* headerName, const char* headerValue)
  {
  ...
  void FServer::_parseArguments(const String& data)
  {
  ...
  void FServer::_uploadWriteByte(uint8_t b)
  {
  ...
  int FServer::_uploadReadByte(NetworkClient& client)
  {
    int res = client.read();
  ...
  bool FServer::_parseForm(NetworkClient& client, const String& boundary, uint32_t len)
  {
    (void)len;
    ...
    if (line == ("--" + boundary))
    {
      ...
      _postArgs = new RequestArgument[FServer_MAX_POST_ARGS];
      _postArgsLen = 0;
      while (1)
      {
        String argName;
        ...
        if (line.length() > (size_t)19 && line.substring(0, 19).equalsIgnoreCase(F("Content-Disposition")))
        {
          ...
          if (nameStart != -1)
          {
            ...
            if (!argIsFile)
            {
              ...
              else if (_postArgsLen >= FServer_MAX_POST_ARGS)
              {
                log_e("Too many PostArgs (max: %d) in request.", FServer_MAX_POST_ARGS);
                return false;
              }
            }
            else
      ...
      int iarg;
      int totalArgs = ((FServer_MAX_POST_ARGS - _postArgsLen) < _currentArgCount) ? (FServer_MAX_POST_ARGS - _postArgsLen) : _currentArgCount;
  ...
  bool FServer::_parseFormUploadAborted()
  {
