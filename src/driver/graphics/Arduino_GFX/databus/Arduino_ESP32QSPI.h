...
public:
  Arduino_ESP32QSPI(
      ...
      int8_t quadwp,
      int8_t quadhd,
      bool is_shared_interface = false);  // Constructor
  ...
  void writePixels(const uint16_t* data, uint32_t len) override;

  void batchOperation(const uint8_t* operations, size_t len) override;
  void writeBytes(const uint8_t* data, uint32_t len) override;
