...
namespace pixeler
{
  ...
  public:
     ...
     * Виводить повідомлення в журнал, якщо файл відсутній.
     *
    ...
    /**
     * @brief Перевіряє чи існує двійковий файл на карті пам'яті.
     * Не виводить повідомлення в журнал, якщо файл відсутній.
     *
     * @param path
     * @return true - якщо файл існує.
     * @return false - якщо файл не існує або за вказаним шляхом знаходиться директорія
     */
    bool fileExistSilently(const char* path);

    /**
     * @brief Перевіряє чи існує папка на карті пам'яті.
     * Виводить повідомлення в журнал, якщо папака не існує.
     *
     * @param path Шлях, вказаний без точки монтування.
    ...
    /**
     * @brief Перевіряє чи існує папка на карті пам'яті.
     * Не виводить повідомлення в журнал, якщо папака не існує.
     *
     * @param path Шлях, вказаний без точки монтування.
     * @return true - якщо папка існує.
     * @return false - якщо папка не існує або за вказаним шляхом знаходиться двійковий файл
     */
    bool dirExistSilently(const char* path);

    /**
     * @brief Перевіряє чи існує ВказівникНаФайл на карті пам'яті за вказаним шляхом.
     * Виводить повідомлення в журнал, якщо ВказівникНаФайл не існує.
     *
     * @param path Шлях, вказаний без точки монтування.
    ...
    /**
     * @brief Перевіряє чи існує ВказівникНаФайл на карті пам'яті за вказаним шляхом.
     * Не виводить повідомлення в журнал, якщо ВказівникНаФайл не існує.
     *
     * @param path Шлях, вказаний без точки монтування.
     * @return true - якщо ВказівникНаФайл існує.
     * @return false - якщо ВказівникНаФайл не існує
     */
    bool existsSilently(const char* path);
     ...
     * @param to Шлях до бінарного файлу, вказаний без точки монтування, куди буде виконана спроба створити копію
     * @return true - якщо задачу було успішно створено та запущено.
     * @return false - якщо вже працює будь-яка інша задача файлового менеджера або не вдалося створити нову задачу за будь-якої причини
     */
     ...
     *
     * @param path Шлях до бінарного файлу, вказаний без точки монтування
     ...
     * @param seek_pos Позиція від початку бінарного файлу у байтах, з якої необхідно розпочати читання
    ...
    /**
     * @brief Читає вказану кількість байтів з відкритого бінарного файлу.
     *
    ...
    /**
     * @brief Читає вміст файлу до рядка.
     *
     * @param path Шлях до бінарного файлу, вказаний без точки монтування
     * @return String - Вміст файлу, якщо читання відбулось успішно.
     ...
     *
     * @param path Шлях до бінарного файлу, вказаний без точки монтування
     * @param mode Режим відкриття бінарного файлу
    ...
    /**
     * @brief Повертає кількість байтів, які лишилися до кінця бінарного файлу від поточної позиції каретки.
     *
     * @param file Вказівник на відкритий бінарний файл
     * @param file_size Розмір файлу у байтах
     * @return size_t - кількість байтів, які лишилися до кінця бінарного файлу від поточної позиції каретки.
     * @return 0 -  якщо операція завершилася невдачею
     */
    size_t available(FILE* file, size_t file_size);

    /**
     * @brief Повертає прогрес операції копіювання бінарного файлу.
     *
     * @return uint8_t - прогрес операції копіювання бінарного файлу у відсотках
     */
    ...
    /**
     * @brief Повертає стан прапора, який вказує на те, чи запущена в даний момент будь-яка із задач файлового менеджера.
     *
    ...
    /**
     * @brief Повертає значення прапора, який вказує чи була успішно завершена остання запущена задача.
     *
     ...
     *
     * @param file_path Ім'я файлу, для якого необхідно створити унікальне для каталогу ім'я
     * @return String
     */
     ...
     *
     * @param bus_mutex Мютекс шини SPI, якщо потрібно
